...
public class PublicMergeSorterTest
{
	...
	private static final int TEST_SIZE = 60_000;
	...
	public void testDivideNSplitsEvenly() throws Exception
	{
		...
		assertTrue(
				((output1.size() == 5) && (output2.size() == 6)) || ((output1.size() == 6) && (output2.size() == 5)));
	}
	...
	public void testLargeReversed()
	{
		...
		Queue<Integer> sorted = ms.mergeSort(reversed);
		assertEquals(TEST_SIZE, reversed.size());
		for (int i = TEST_SIZE - 1; i >= 0; i--)
		{
			assertEquals((Integer) i, reversed.dequeue());
		}
		for (int i = 0; i < TEST_SIZE; i++)
		{
			assertEquals((Integer) i, sorted.dequeue());
		}
	}
}
